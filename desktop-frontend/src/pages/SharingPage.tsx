...
function ProfileEditor({open, profile, fleet, onClose, onSaved, runTask, notify}: {
  open: boolean;
  profile?: SharingProfile;
  fleet: FleetState;
  onClose: () => void;
  onSaved: (profile: SharingProfile) => Promise<unknown>;
  ...
  const submit = async () => {
    ...
    if (!await runTask(async () => {
      ...
      if (result.profile) await onSaved(result.profile);
      onClose();
    })) return;
...
function ProfileDetail({profile, fleet, refreshFleet, runTask, notify, onEdit}: {
  profile: SharingProfile;
  fleet: FleetState;
  ...
  const action = async (kind: 'enable' | 'disable' | 'delete') => {
    ...
    if (!await runTask(async () => {await operation(profile.id); await refreshFleet();}, confirmation)) return;
    notify(kind === 'enable' ? '共享方案启用请求已完成，请检查诊断结果。' : kind === 'disable' ? '共享方案恢复请求已完成，请检查原网络。' : '共享方案已删除。');
  };

  const probe = async () => {
    if (!await runTask(async () => setProbes((await probeHosts([profile.gateway_id, ...profile.client_ids])).results || []))) return;
    notify('已完成所选主机的实际探测。');
  };

  const helper = async () => {
    if (!await runTask(async () => {await installHelper([profile.gateway_id, ...profile.client_ids]);}, {title: '安装远端辅助程序？', body: '将在所选出口机与客户端安装网络辅助程序，需要远端管理员权限。请确认主机选择和 SSH 指纹无误。'})) return;
    notify('辅助程序安装请求已完成，请再次探测确认各主机能力。');
  };

  return <>
    <Surface title="方案详情" description="先查看路径和影响范围，再选择启用、停止或恢复。" action={<Button appearance="subtle" onClick={onEdit}>编辑方案</Button>}>
      ...
      <div className="page-grid">
        <div><p className="section-kicker">基本信息</p><dl className="definition-grid"><div><dt>状态</dt><dd><StatusPill level={active ? 'ok' : profile.state === 'error' || profile.cleanup_pending ? 'danger' : 'info'}>{profile.cleanup_pending ? '恢复未完成' : profile.state}</StatusPill></dd></div><div><dt>方案 ID</dt><dd className="mono">{profile.id}</dd></div><div><dt>出口机</dt><dd>{gateway}</dd></div><div><dt>客户端</dt><dd>{clientNames.join('、') || '—'}</dd></div></dl></div>
      ...
      <div className="form-actions" style={{justifyContent: 'flex-start'}}><ButtonRow><Button id="enable-profile" appearance="primary" disabled={active} icon={<PlugConnected24Regular />} onClick={() => void action('enable')}>启用共享</Button><Button id="disable-profile" appearance="outline" disabled={!active && !profile.cleanup_pending} icon={<Warning24Regular />} onClick={() => void action('disable')}>停止并恢复</Button><Button id="delete-profile" appearance="subtle" disabled={active || profile.cleanup_pending} onClick={() => void action('delete')}>删除方案</Button></ButtonRow></div>
    </Surface>
    <Surface title="健康与恢复" description="实际探测用于确认主机能力；探测结果不自动触发危险操作。">
      <ButtonRow><Button id="probe-all" appearance="subtle" icon={<Search24Regular />} onClick={() => void probe()}>探测出口与客户端</Button><Button id="install-helper" appearance="subtle" onClick={() => void helper()}>安装辅助程序</Button></ButtonRow>
      <div id="probes" className="event-list" style={{marginTop: 14}}>{!probes.length ? <EmptyState>尚未执行本次方案探测。</EmptyState> : probes.map((probe, index) => <div className="event-item" key={`${probe.id || 'probe'}-${index}`}><strong>{hostName(String(probe.id || '未知主机'))}</strong><p>{probe.ok === true ? '实际探测成功。' : '实际探测未通过。'} <span className="mono">{JSON.stringify(probe)}</span></p></div>)}</div>
...
export function SharingPage({fleet, refreshFleet, runTask, notify}: SharingPageProps) {
  ...
  const onSaved = async (profile: SharingProfile) => {await refreshFleet(); setSelectedId(profile.id);};
  ...
  return <>
    ...
    <div className="page-grid page-grid-wide sharing-layout">
      ...
      <div id="profile-detail">{selected ? <ProfileDetail profile={selected} fleet={fleet} refreshFleet={refreshFleet} runTask={runTask} notify={notify} onEdit={() => openEditor(selected)} /> : <Surface title="方案详情"><EmptyState>选择一个方案查看路径、出口机、客户端和恢复状态。</EmptyState></Surface>}</div>
    </div>
    <ProfileEditor open={editorOpen} profile={editorProfile} fleet={fleet} onClose={() => setEditorOpen(false)} onSaved={onSaved} runTask={runTask} notify={notify} />
  </>;
}
