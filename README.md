...
> 当前版本为 `0.7.0-alpha`。网络切换属于高风险运维操作，请先在可现场恢复的测试机上验证。不要把管理台直接暴露到公网。
...
安装、启动、主机导入、借网、恢复、Clash/TUN、桌面面板和开发验证命令集中收录在 [命令大全](命令大全.md)。

面向终端客户的订阅制纯享版、校园局域网指纹识别、中继隐藏、服务端限速和分阶段交付方案见 [借网客户端软件开发规划表](借网客户端软件开发规划表.md)。

客户纯享版 MVP 已提供独立入口、签名订阅、线路状态、延迟检测、借网与退网复检。使用和签发格式见 [客户纯享版文档](docs/CLIENT.md)。该版本面向获授权的个人购买宽带出口，不把运营商套餐与学校免费校园网额度混为一谈。

商业服务端现已提供在线开户注册、设备签名认证、短期租约、服务端撤销、WireGuard 实际字节计量及 Linux 中继限速。部署、运营命令和中继安装见 [商业借网服务手册](docs/COMMERCIAL_SERVICE.md) 与 [中继执行器说明](docs/CLIENT_RELAY.md)。

## 桌面面板

安装后运行 `server-network-assist-desktop`。Windows 安装程序创建“服务器网络助手”桌面快捷方式，双击即开。面板可独立添加出口机和客户端、创建网络共享方案、选择是否共享源 HTTP 代理，并直接控制已有隧道。关闭窗口或退出托盘不会断网。所有界面资源本地提供，无需 WSL；完整离线包包含 SSH、加密和托盘依赖。
...
| Ubuntu 出口机为 Windows 和 Ubuntu 客户端提供网络 | 桌面面板 → 共享网络；或管理台 → 网络借助 | [案例二](#案例二一台-ubuntu-出口机服务两种系统) |
...
已经配置好 WireGuard 的机器，可以用桌面面板完成日常检查。新版还可在“主机与凭据”添加机器，在“共享网络”创建和恢复方案，无需打开管理台。
...
**关闭窗口不会断网。** “本机隧道”的连接开关只控制已有服务；完整结束桌面创建的借网方案并清理临时路由，请在“共享网络”使用“断开并恢复原网络”。其他管理台创建的方案应在原管理库恢复。原生托盘提供打开面板、网络变化通知和退出托盘；面板不包含 Clash 订阅及规则编辑。

### 案例四：在桌面独立创建多机共享

在“主机与凭据”添加 Windows 或 Ubuntu 出口机及客户端，核对 SSH 指纹；在“共享网络”选择这些机器，并选择仅共享网络或共享源 HTTP/混合代理端口。保存、探测后启用，失败时按“诊断与恢复”指引处理。支持的系统组合和代理作用范围见 [共享指南](docs/SHARING.md)。

[![新版桌面共享网络流程（隔离测试数据）](docs/images/desktop-sharing-v030.png)](docs/images/desktop-sharing-v030.png)
