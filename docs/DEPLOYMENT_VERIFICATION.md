...
- 登录后打开“代理与 TUN”，逐台刷新已配置主机，确认未安装、未运行、Controller 不可用和正常运行四种状态均显示明确原因。
- 正常主机应显示核心版本、模式、mixed-port、TUN、系统代理及策略组当前节点；长配置路径、规则和错误信息可完整滚动查看。
- 所有修改操作必须二次验证管理员密码。模式、TUN 和规则操作应留下远端备份；加载失败应恢复原配置。
- 策略组节点只能从 Controller 返回的候选节点中选择。Controller 只允许目标机本地回环地址或 Windows 命名管道。
- Windows 验证系统代理开关；Ubuntu SSH 会话应明确显示系统代理不支持，不能声称修改了桌面用户设置。
