...
function render(s) {
  ...
  $('network-dot').className = 'badge ' + (ok ? 'color-green' : 'color-orange');
  ...
  for (const tunnel of s.tunnels) {
    const card = document.createElement('article'); card.className = 'tunnel card';
    const content = document.createElement('div'); content.className = 'card-content card-content-padding'; const header = document.createElement('div'); header.className = 'card-header';
    const name = document.createElement('h3'); name.className = 'no-margin'; name.textContent = tunnel.name;
    const pill = document.createElement('span'); pill.className = 'badge' + (tunnel.active ? ' color-green' : ''); pill.textContent = tunnel.active ? '已连接' : '已断开';
    ...
    const button = document.createElement('button'); button.textContent = tunnel.active ? '断开连接' : '连接'; button.className = 'button button-round ' + (tunnel.active ? 'button-outline' : 'button-fill');
    button.disabled = !s.elevated; button.onclick = () => action(tunnel.active ? 'disconnect' : 'connect', tunnel.name);
    button.classList.toggle('disabled', button.disabled);
    const footer = document.createElement('div'); footer.className = 'card-footer'; footer.append(button);
    card.append(header, content, footer); $('tunnels').append(card);
  }
  if (!s.tunnels.length) { const empty = document.createElement('div'); empty.className = 'block'; empty.textContent = '尚未发现已安装的 WireGuard 隧道。可先通过服务器管理台配置借网客户端。'; $('tunnels').append(empty); }
  ...
  $('disable-proxy').classList.toggle('disabled', $('disable-proxy').disabled);
