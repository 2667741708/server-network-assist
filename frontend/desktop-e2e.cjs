...
(async()=>{
  ...
  try{
    ...
    for(const [name,width,height]of [['desktop',1280,960],['mobile',390,844]]){
      ...
      await nav('hosts-page');await page.locator('#credentials-tab').click();await page.locator('#credential-name').fill('主机连接凭据');await page.locator('#credential-password').fill('test-password-not-real');await page.getByRole('button',{name:'保存凭据',exact:true}).click();await settled('凭据已加密保存');assert.equal(await page.locator('#credential-password').inputValue(),'');await page.locator('#hosts-tab').click();
      ...
      await checkLayout('hosts');await page.screenshot({path:path.join(root,'artifacts',`desktop-hosts-${name}.png`),fullPage:true});
      ...
      await page.getByRole('button',{name:'恢复',exact:true}).first().click();await page.getByRole('button',{name:'确认',exact:true}).click();await settled('代理备份已恢复');assert.equal(proxy.enabled,false);await checkLayout('proxy');await page.screenshot({path:path.join(root,'artifacts',`desktop-proxy-${name}.png`),fullPage:true});
      ...
      await nav('diagnostics');await page.getByRole('button',{name:'导出诊断记录',exact:true}).waitFor();await page.getByText('联网正常',{exact:true}).waitFor();const download=page.waitForEvent('download');await page.getByRole('button',{name:'导出诊断记录',exact:true}).click();assert.match((await download).suggestedFilename(),/network-diagnostics/);await checkLayout('diagnostics');await page.screenshot({path:path.join(root,'artifacts',`desktop-diagnostics-${name}.png`),fullPage:true});
